...
def render_gene_explorer_page(df, model):
    ...
    if model is not None:
        ...
        gene_df['predicted'] = model.predict(X_gene)
                ...
                # --- CLINICAL TIERING (load pre-computed tiers) ---
        ...
        if tier_path.exists():
            ...
            tier_df = pd.read_csv(
                tier_path, 
                compression='gzip',
                usecols=['chrom', 'pos', 'tier', 'action', 'promoter_gene']
            )
            # Filter to only the gene we're looking at
            tier_df = tier_df[tier_df['promoter_gene'] == gene_input]
            
            # --- FIX: Deduplicate before merging ---
            # Keep only the first occurrence of each variant
            tier_df = tier_df.drop_duplicates(subset=['chrom', 'pos'], keep='first')
            
            # Create merge keys
            gene_df['merge_key'] = gene_df['chrom'].astype(str) + '_' + gene_df['pos'].astype(str)
            tier_df['merge_key'] = tier_df['chrom'].astype(str) + '_' + tier_df['pos'].astype(str)
            
            # Merge using the key
            gene_df = gene_df.merge(
                tier_df[['merge_key', 'tier', 'action']], 
                on='merge_key', 
                how='left'
            )
            gene_df = gene_df.drop(columns=['merge_key'], errors='ignore')
            
        else:
            # Fallback: compute simple tiers
            def assign_tier_simple(row):
                slope = abs(row['predicted'])
                if slope > 0.50:
                    return ('Tier 1: High-Impact', 'Consider functional validation.')
                elif slope > 0.20:
                    return ('Tier 2: Moderate Impact', 'Periodic re-evaluation recommended.')
                else:
                    return ('Tier 3: Regulatory Benign', 'Standard clinical follow-up.')
            gene_df[['tier', 'action']] = gene_df.apply(assign_tier_simple, axis=1, result_type='expand')
        
        # Fill missing tiers
        gene_df['tier'] = gene_df['tier'].fillna('Tier 3: Regulatory Benign')
        gene_df['action'] = gene_df['action'].fillna('Standard clinical follow-up.')
        
        # Tier counts
        tier_counts = gene_df['tier'].value_counts()
        
        col1, col2, col3 = st.columns(3)
        col1.metric("🔴 Tier 1 (High Impact)", len(gene_df[gene_df['tier'].str.contains('Tier 1')]))
        col2.metric("🟡 Tier 2 (Moderate)", len(gene_df[gene_df['tier'].str.contains('Tier 2')]))
        col3.metric("🟢 Tier 3 (Benign)", len(gene_df[gene_df['tier'].str.contains('Tier 3')]))
        
        # --- Tier Summary Table (limit to first 15) ---
        st.markdown("#### Variant Summary")
        
        summary_lines = ["| Badge | Tier | Gene | Predicted Slope | Action |"]
        summary_lines.append("|-------|------|------|-----------------|--------|")
        
        for _, row in gene_df.head(15).iterrows():
            tier = row.get('tier', 'Tier 3: Regulatory Benign')
            if "Tier 1" in tier:
                badge = "🔴"
            elif "Tier 2" in tier:
                badge = "🟡"
                ...
                badge = "🟢"
            slope = row.get('predicted', 0.0)
            action = row.get('action', 'Standard clinical follow-up.')
            summary_lines.append(f"| {badge} | {tier} | {row['promoter_gene']} | {slope:.3f} | {action[:40]}... |")
        
        st.markdown("\n".join(summary_lines))
        
        # --- Tier Distribution Pie Chart (only if data exists) ---
        st.markdown("#### Tier Distribution")
        if len(tier_counts) > 0 and tier_counts.sum() > 0:
            fig = px.pie(
                names=tier_counts.index,
                values=tier_counts.values,
                hole=0.4,
                color_discrete_map={
                    'Tier 1: Actionable High-Impact': '#FF3366',
                    'Tier 1: High-Impact': '#FF6633',
                    'Tier 2: Moderate Impact': '#00F2FE',
                    'Tier 3: Regulatory Benign': '#00FF87'
                }
            )
            fig.update_layout(
        ...
        # CSV Export
        st.markdown("---")
        csv_data = gene_df_display.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="📥 Download CSV",
            data=csv_data,
