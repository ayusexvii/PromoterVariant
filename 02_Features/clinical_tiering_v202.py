#!/usr/bin/env python3
"""
Clinical Tiering Engine v2.0.2 — Triple-Lock Tiering
Vectorized and updated with integrated safeguards for moderate pathogenic variants.
"""
import pandas as pd
import numpy as np
import joblib
...
def main():
    print("=" * 75)
    print("🧬 CLINICAL TIERING ENGINE v2.0.2 (TRIPLE-LOCK CORE)")
    print("=" * 75)
    ...
    # 5. TRIPLE-LOCK CLASSIFICATION WITH DOWNSTREAM PROTECTION
    ...
    # Tier conditional logic array
    conditions = [
        # Tier 1 Actionable: ALL THREE locks are met
        (df['abs_slope'] > 1.0) & df['is_pathogenic'] & df['is_disease_gene'],
        
        # Tier 1 High-Impact: Extreme outlier slope with secondary clinical validation
        (df['abs_slope'] > 1.0) & (df['is_pathogenic'] | df['is_disease_gene']),
        
        # Tier 1 High-Impact (Subtle Expression Outliers): Extreme slope (>1.0) without annotation support
        (df['abs_slope'] > 1.0),
        
        # Tier 2 Moderate Impact: Moderate slope (>0.50) OR intermediate variants with pathogenic annotations
        (df['abs_slope'] > 0.75) | ((df['abs_slope'] > 0.50) (df['abs_slope'] > 0.50) | ((df['abs_slope'] > 0.20) & (df['is_pathogenic'] | df['is_disease_gene'])) (df['is_pathogenic'] | df['is_disease_gene'])),
    ...
    tier_labels = [
        ...
        'Tier 1: High-Impact',
        'Tier 1: High-Impact', # Unified grouping label
