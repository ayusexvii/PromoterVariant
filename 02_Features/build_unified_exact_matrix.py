...
Fully vectorized, handles missing variables cleanly, and guards data types.
"""
import pandas as pd
import numpy as np
import sys
...
def main():
    print("=" * 75)
    print("🧬 ACCELERATED UNIFIED MATRIX COMPILATION ENGINE (ALL TISSUES)")
    print("=" * 75)
    ...
    for name, (path, is_required) in tissue_files.items():
        ...
        if path.exists():
            ...
            # Inject tracking labels if missing from custom source files
            if 'clinvar_gene' not in tdf.columns:
                tdf['clinvar_gene'] = 'TERT' if 'tert' in path.name.lower() else ('HBB' if 'hbb' in path.name.lower() else 'UNKNOWN')
            if 'clinvar_sig' not in tdf.columns:
                tdf['clinvar_sig'] = 'Validated'
                
            print(f"   📊 Ingested {len(tdf):,} rows.")
    ...
    # 2. Concat matrices using true missing value references (No 'NA' strings)
    ...
    # 3. Load Upstream Features Safe Frames
    ...
    # Normalize coordinate strings to avoid prefix mismatches
    combined['chrom'] = combined['clinvar_chrom'].astype(str).str.replace('chr', '', case=False)
    combined['pos'] = combined['clinvar_pos'].astype(int)
    ...
    # 6. Extract Conservation Metadata (Replaces broken zip maps)
    if cons_path.exists():
    ...
    # Safely locate the eQTL slope column name variation
    ...
    # 8. Clean up incomplete data entries using real missing value definitions
    initial_count = len(train_df)
    train_df.dropna(subset=feature_cols + ['eqtl_slope'], inplace=True)
