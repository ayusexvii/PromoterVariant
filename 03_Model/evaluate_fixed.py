#!/usr/bin/env python3
"""
Model Evaluation with Diagnostic Plots - FIXED VERSION
Loads unified feature matrix with target column.
"""
import pandas as pd
import numpy as np
import joblib
...
def load_data(features_path: str = "processed/feature_matrix_chr22.csv.gz",
              model_path: str = "processed/baseline_model.pkl"):
    """Load unified feature matrix (features + target) and model."""
    logger.info("📂 Loading unified feature matrix...")
    
    try:
        # Load unified dataframe (contains features AND target)
        ...
        # Remove rows with NaN in target (unmatched variants)
        mask = ~np.isnan(y)
        X = X[mask].reset_index(drop=True)
        y = y[mask]
        
        logger.info(f"✅ Matched variants: {len(X):,} samples, {X.shape[1]} features")
...
def generate_evaluation_plots(X_test, y_test, y_pred, model, output_dir: str = "processed"):
    ...
    axes[1, 0].hist(residuals, bins=20, edgecolor='black', alpha=0.75, color='mediumseagreen')
...
def main(features_path: str = "processed/feature_matrix_chr22.csv.gz",
         model_path: str = "processed/baseline_model.pkl"):
    logger.info("=" * 80)
    logger.info("📊 MODEL EVALUATION & DIAGNOSTICS (FIXED)")
    logger.info("=" * 80)
    
    X, y, model = load_data(features_path, model_path)
    ...
    if len(residuals) > 10:
        ...
        if abs(corr) > 0.35:
            logger.warning(f"⚠️  Possible heteroscedasticity detected (corr={corr:.3f})")
        else:
            logger.info(f"✅ Residuals appear well-behaved (corr={corr:.3f})")
